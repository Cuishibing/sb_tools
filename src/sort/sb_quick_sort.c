...
void quick_sort_real(sb_element *array, int start, int end) {
    ...
    while (left_index <= right_index) {
        while (left_index < end && sb_compare(array[left_index], array[end]) < 0) {//left_index<end
            left_index++;
        }
        while (right_index > -1 && sb_compare(array[right_index], array[end]) > 0) {//right_index>=end
            right_index--;
        }
